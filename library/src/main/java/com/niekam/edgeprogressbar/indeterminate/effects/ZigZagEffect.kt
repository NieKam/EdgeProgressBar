...
class ZigZagEffect : Effect {
  ...
  override fun start() {
    ...
    mIndeterminateAnimation = ObjectAnimator.ofFloat(
        (contract.getTotalLength() * DASH_LINE_COUNT),
        0F)
    ...
    val mediatePrimaryColor = argbEvaluator.evaluate(.85F, mColorPrimary, mColorSecondary)
    val mediateSecondaryColor = argbEvaluator.evaluate(.45F, mColorSecondary, mColorPrimary)

    mPaint.color = mediatePrimaryColor as Int
    mDashPaint.color = mediateSecondaryColor as Int
  }
